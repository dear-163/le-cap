// 首頁「RSI超賣＋成交量暴增」篩選器。結果由worker-cron每天算好存進daily_screener_signals，
...
export async function onRequestGet(context) {
  ...
  try {
    ...
    const { results } = await env.ELAN_QUANT_DB
      .prepare('SELECT code, name, rsi, volume_ratio, close FROM daily_screener_signals WHERE date = ? ORDER BY volume_ratio DESC')
      .bind(latestDate)
      .all();

    const response = json({
      date: latestDate,
      criteria: 'RSI(14) < 30，且成交量 ≥ 前5個交易日均量的3倍（暴增200%以上）',
    ...
    return json({ error: `查詢RSI超賣＋成交量暴增篩選器失敗：${error.message}` }, 500);
  }
}
