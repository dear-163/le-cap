...
4. 這個 Workers 專案的 Settings 也綁定同一個 D1（Variable name 一樣要叫 `ELAN_QUANT_DB`，跟 `worker-cron/wrangler.toml` 裡的 binding 名稱一致），確認 Cron Trigger（`0 10 * * 1-5`，平日台灣時間18:00）已啟用。
